...
function kindBadgeClass(kind: ActivityItemType['kind'], reviewState?: ActivityItemType['reviewState']) {
  switch (kind) {
    case 'issue_opened':
  ...
  return (
    <article className="rounded-2xl border border-zinc-200/70 bg-white/50 p-4 shadow-sm backdrop-blur dark:border-zinc-800/70 dark:bg-zinc-900/30">
      ...
      <h3 className="mt-2 text-base font-medium leading-snug text-zinc-900 dark:text-zinc-100">
        <a className="no-underline hover:underline" href={item.url} target="_blank" rel="noreferrer">
          {item.title}
        </a>
      </h3>
      ...
      {item.summary
        ? (
            <p className="mt-2 text-sm leading-relaxed text-zinc-700 dark:text-zinc-300">{item.summary}</p>
          )
        : null}
    </article>
  )
