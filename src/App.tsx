    ...
    <div className="min-h-screen bg-zinc-50 text-zinc-900 dark:bg-zinc-950 dark:text-zinc-100">
      ...
      <div className="mx-auto max-w-4xl px-6 py-12">
        <header className="space-y-6">
          <div className="flex flex-col items-start justify-between gap-6 sm:flex-row sm:items-center">
            <div className="min-w-0">
              ...
              <p className="mt-2 text-sm leading-relaxed text-zinc-700 dark:text-zinc-300">{subtitle}</p>
            </div>
          ...
          <div className="flex flex-wrap items-center gap-4 text-sm text-zinc-600 dark:text-zinc-400">
            ...
            {state.status === 'loaded'
              ? <span className="font-mono text-xs sm:text-sm">updated: {formatGeneratedAt(state.data.generatedAt)}</span>
              : null}
          </div>
          ...
          {state.status === 'error'
            ? (
                <div className="space-y-3 rounded-2xl border border-red-200/70 bg-white/50 p-4 text-sm text-zinc-700 shadow-sm backdrop-blur dark:border-red-900/40 dark:bg-zinc-900/30 dark:text-zinc-300">
                  ...
                  <p className="break-words font-mono text-[11px] text-zinc-500 dark:text-zinc-500">{state.message}</p>
                  <p className="text-xs text-zinc-600 dark:text-zinc-400">
                    For local development, start the Worker API in another terminal:{' '}
                    <code className="rounded bg-zinc-100 px-1 py-0.5 dark:bg-zinc-950/40">pnpm worker:dev</code>
                  </p>
                </div>
              )
            : null}
        ...
        <footer className="mt-14 border-t border-zinc-200/70 pt-6 text-xs text-zinc-500 dark:border-zinc-800/70 dark:text-zinc-500">
          ...
          © 2026 Yuki Okushi. All Rights Reserved.
          </p>
        </footer>
      </div>
    </div>
